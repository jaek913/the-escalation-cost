...
**AMENDMENT 2026-07-26 (Phase-5a adversarial-review finding F-07; author-ratified; frozen BEFORE the amendment run).** The rule's ordering conjunct ("point estimate >= components") is a bare comparison of two statistics computed from the same seventeen observations, with no measure of whether the gap exceeds its own sampling noise - the same defect class the E14 record diagnoses (a comparison that discards the covariance the resampling machinery preserves), applied here as a point reading rather than a marginal-interval reading. THE CURE, specified in full before the run: the paired contrast, computed WITHIN the same outcome-permutation machinery the p-value already uses. Construction: for each of B = 2000 permutations of the realized outcomes (dedicated generator, seed 20260714 = the frozen run seed + 1, so the existing three tests' draws are untouched and every committed E2 value reproduces exactly), compute s_D, s_rho-crisis, and s_|dphi| against the SAME permuted outcome - the three statistics are thereby paired within every resample and their covariance is preserved. Observed contrasts: c_rho = s_D - s_rho-crisis and c_dphi = s_D - s_|dphi|; one-sided permutation p per contrast = (#{c_pi >= c_obs} + 1)/(B + 1). COMMITTED READING, result-independent: a contrast is RESOLVED-POSITIVE if c_obs > 0 with p < 0.05; RESOLVED-NEGATIVE if c_obs < 0 with the mirrored one-sided p < 0.05; UNRESOLVED otherwise. The verdict formula itself is UNCHANGED - rewriting the verdict rule with the result known would be the worse process - but the REPORT is amended: the manuscript states each contrast with its uncertainty and resolution status, and if either contrast is not resolved-positive, Section 6.1's ordering sentence is stated as point-estimate-descriptive with the verdict's inferential weight resting on the association leg. Reported whichever way it lands. Would this amendment be made if it pushed the other way? It IS the direction-neutral version of the question the frozen conjunct already asked - it can only downgrade the conjunct's reading from asserted to unresolved or confirm it with uncertainty attached; it cannot manufacture support. Mechanism validation: the E2 suite gains a fourth leg (planted dominant-predictor -> both contrasts resolved-positive; planted rank-equal predictor -> unresolved, the tie case; exchangeable-predictor false-positive rate at the 0.05 reading measured over independent null panels, band 0.02-0.09) run to ALL PASS before the single amendment run. New ledger family LB-E2-contrast (six rows: both contrasts, both p-values, both readings) declared in OUTLINE per the three-file rule.
...
|---|---|---|---|---|---|
| 1 retail | MRTSSM44000USS | Retail Sales: Retail Trade | Millions of Dollars | Seasonally Adjusted | 1992-01 to 2026-05 |
...
| 3 shipments | AMTMVS | Manufacturers' Value of Shipments: Total Manufacturing | Millions of Dollars | Seasonally Adjusted | 1992-01 to 2026-05 |
...
| sector arm | DGORDER | Manufacturers' New Orders: Durable Goods | Millions of Dollars | Seasonally Adjusted | 1992-02 to 2026-05 |

**COVERAGE COLUMN CORRECTED 2026-07-25 AGAINST THE PULLED FILES.** As first written this column was transcribed from each FRED series PAGE, and two cells were wrong as a result: retail was recorded ending 2026-03 and the sector arm ending 2026-04, while AMTMVS was left as "confirm at pull". Every one of the five files actually ends 2026-05. The pages were serving older vintages than the CSV endpoint delivers, which is not a FRED defect but the ordinary behaviour of a revisable series - and it is the lesson of 19.2 arriving from a new direction: a page is a rendering, the pulled file is the artifact, and only the artifact is evidence. The dates above are now read from the hashed files themselves.
...
- **Option A - reuse the frozen three, pull only the two new.** Preserves three already-committed SHA256 values untouched. The vintage mixture is absorbed by the operator's existing common-overlapping-sample rule, which truncates the analysis window to the EARLIEST common end date. Requires the write-up to state that chain inputs carry two pull dates.
...
### 19.6 Realised sample window (records a consequence, changes no operator)
