...
The hysteresis result splits, and the split is the finding: the raise strategy survives permanent customer attrition in genuinely shifted, persistent regimes, and fails in noisy ones where the estimator's own variance drives the policy on and off. The fourth is the sharpest and is stated as a limit rather than a caveat: under drifting persistence, no estimator rescues the recipe - including an oracle handed the true parameter at every step. That result forecloses the obvious fix. If the remedy failed only because estimation was noisy, better estimation would help; it does not, because the policy is calibrated to a parameter that no longer holds still (S-8, L-04).
