...
def main() -> int:
    ...
    # The banner is followed by a BLANK LINE, which is load-bearing: pandoc only
    # recognises a YAML metadata block that either opens the file or is preceded
    # by a blank line. Without it the manuscript's front-matter "---" is read as
    # a setext underline, the title block is never built, and the YAML prints as
    # body text on page 1 of the PDF. Found by the Phase 5c PDF QA gate and
    # verified against pandoc directly before the fix was made (DECISIONS 131).
